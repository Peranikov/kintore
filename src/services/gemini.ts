...
// トレーニング評価を生成
export async function generateWorkoutEvaluation(log: WorkoutLog): Promise<{ text: string; analysis: WorkoutAnalysisSnapshot }> {
  const apiKey = await getApiKey()
  if (!apiKey) {
    throw new Error('APIキーが設定されていません。設定画面でAPIキーを入力してください。')
  }

  const [profile, model, analysis] = await Promise.all([
    getUserProfile(),
    getGeminiModel(),
    buildWorkoutAnalysis(log),
  ])

  const prompt = `あなたは経験豊富なパーソナルトレーナーです。
ユーザーの今日のトレーニング分析結果を要約し、フィードバックを提供してください。

【重要な制約】
1. 与えられた分析結果に含まれる事実だけを使ってください
2. 数値の比較結果を優先して評価してください
3. 種目別の進歩・横ばい・低下を簡潔に触れてください
4. 最後に次回への具体的なアドバイスを1〜2個含めてください
5. JSONやMarkdown見出しは使わず、短い自然文だけを返してください

${profile ? `■ ユーザープロフィール\n${profile}\n` : ''}
■ 分析結果
${formatAnalysisForPrompt(log, analysis)}

【出力形式】
・簡潔で具体的なフィードバック（180-260文字程度）
・ポジティブな点と改善点をバランスよく
・絵文字は使わない`
